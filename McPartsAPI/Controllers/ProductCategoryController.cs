...
namespace McPartsAPI.Controllers
{
    ...
    public class ProductCategoryController : ControllerBase
    {
        ...
        [HttpGet]
        public async Task<ActionResult<IEnumerable<productcategorydtoGet>>> GetAll()
        {
            Expression<Func<productcategory, bool>> expression = p => p.isdeleted == false;
            var data = await _service.GetListByExpressionAsync(expression);
            var requestpayload = _mapper.Map<List<productcategorydtoGet>>(data);
        ...
        {
            Expression<Func<productcategory, bool>> expression = p => p.isdeleted == false && p.id == id;
            var data = await _service.GetSingleEntityByExpressionAsync(expression);
            var requestpayload = _mapper.Map<productcategorydtoGet>(data);
            return Ok(requestpayload);
        }

        [HttpPost]
        [Route("create")]
        public async Task<ActionResult<bool>> Create([FromBody] productcategorydto data)
        {
        ...
        {
            var requestpayload = _mapper.Map<productcategorydto>(data);
        ...
        {
            Expression<Func<productcategory, bool>> expression = p => p.id == id;
